...
async def refresh_snapshot() -> dict:
    ...
    async with _snapshot_lock:
        try:
            results = await asyncio.gather(*(client.fetch_source(source, date.today()) for source in SOURCES))
            fresh = build_snapshot(results, datetime.now(timezone.utc))
            store.save(fresh)
            _snapshot = fresh
            _last_error = None
            return fresh
            ...
            print(f"DataSF refresh failed: {_last_error}", flush=True)
            if _snapshot:
                return _snapshot
            raise
...
async def health() -> JSONResponse:
    return JSONResponse({"ok": True, "version": APP_VERSION, "has_snapshot": bool(_snapshot), "generated_at": (_snapshot or {}).get("generated_at"), "last_error": _last_error})


@app.get("/api/refresh")
async def manual_refresh() -> JSONResponse:
    fresh = await refresh_snapshot()
    return JSONResponse({"ok": True, "generated_at": fresh.get("generated_at")})
